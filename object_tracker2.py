...
def main():
    ...
    cap = cv2.VideoCapture(2)  # Adjust to 0 for laptop camera or other index for Iriun

    if not cap.isOpened():
        print("Error: Could not open Iriun Webcam. Ensure the Iriun client is running and USB is connected.")
        print("Try changing the device index (e.g., 0, 1, 2) or check /dev/video* devices.")
        return
    ...
    template = cv2.cvtColor(template, cv2.COLOR_BGR2GRAY)  # Convert to grayscale for matching
